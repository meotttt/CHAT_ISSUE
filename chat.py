...
import re
import asyncio
# import sqlite3  # <- Удаляем sqlite3
from datetime import datetime, timedelta, timezone

import psycopg2  # <- Добавляем psycopg2 для PostgreSQL
from psycopg2 import sql # Для безопасного составления SQL запросов
from psycopg2.extras import DictCursor # Для доступа к данным по имени столбца
...
# --- Общая Конфигурация ---
# TOKEN = "8086930010:AAH1elkRFf6497_Ls-XnZrUeIh_rWyMF5c"  # ЗАМЕНИТЕ НА СВОЙ ТОКЕН!
TOKEN = os.environ.get("TELEGRAM_BOT_TOKEN") # Получаем токен из переменной окружения
if not TOKEN:
    raise ValueError("TELEGRAM_BOT_TOKEN не установлен в переменных окружения!")

# DATABASE_URL будет предоставлен Railway PostgreSQL Add-on
...
# Использование относительного пути, предполагая, что папка 'photo' находится в корне проекта
PHOTO_BASE_PATH = "photo" # Относительный путь к папке с фотографиями
NUM_PHOTOS = 74
# USER_DATA_FILE = "user_data.json"  # Больше не используется, данные Лависки будут в PostgreSQL
