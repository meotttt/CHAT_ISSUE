...
async def lav_iska(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if current_time - last_time < last_cd:
        ...
        await update.message.reply_text(f"⏳ Вы использовали 🧧 и получили уникальную loveisку. Повторите через {' '.join(parts)}")
        return
...
async def unified_text_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user and not user.is_bot:
        ...
        # --- Команды Брачного Бота ---
            ...
            initiator_info = await get_user_info(update.message.from_user.id)
...
# Расширенная проверка: initiator_info должен быть словарем и содержать ключ 'id'
    ...
    # Отправляем более информативное сообщение об ошибке
                await update.message.reply_text("Произошла ошибка при получении данных инициатора (пользователь не найден в базе данных или данные повреждены).")
                return
